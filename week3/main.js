...
function inventorsNames(item) {
 var fullName = [item.first, item.last ].join(" ");
 return fullName;
}

function printNames() {
    const inventorName = inventors.map(inventorsNames);
    return inventorName;
}
console.table(printNames());
...
const ageOfInventors = data => data.reduce((acc, item) => {
    acc[ item.last] = {age: item.passed - item.year};
    return acc;
}, {});

const ages = ageOfInventors(inventors);
console.table(ages);
...
let countedData = data.reduce(function (allData,dataName) {
    if(dataName in allData) {
        allData[dataName]++
    }
    else {
        allData[dataName] = 1;
